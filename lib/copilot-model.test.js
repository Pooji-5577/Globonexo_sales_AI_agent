...
describe("wizard shape", () => {
  ...
  it("shows the calling step for voice and both", () => {
    expect(visibleSteps(draftWith({ channel: "voice" })).map(s => s.id)).toContain("voice");
    expect(visibleSteps(draftWith({ channel: "both" })).map(s => s.id)).toContain("voice");
  });
});
...
describe("step blockers", () => {
  it("blocks on the connections step when Gmail is missing for an email campaign", () => {
    ...
    expect(blockers.join(" ")).toMatch(/Connect Gmail/i);
  });
  ...
  it("never blocks on a disconnected calendar", () => {
    const blockers = blockersForStep("connections", {
      draft: draftWith({ channel: "both" }),
      integrations: { ...connected, calendar: { connected: false, status: "disconnected" } },
      leadCount: 5,
    });
    expect(blockers).toHaveLength(0);
  });
...
describe("warnings", () => {
  it("warns about a disconnected calendar without blocking", () => {
    const warnings = warningsForDraft({
      draft: draftWith({}),
      integrations: { ...connected, calendar: { connected: false, status: "disconnected" } },
    });
    expect(warnings.join(" ")).toMatch(/cannot book one into your calendar/i);
  });
...
describe("draft hydration", () => {
  ...
  it("restores a saved draft so the wizard resumes where it stopped", () => {
    ...
    expect(draft.channel).toBe("both");
...
describe("confirmation summary", () => {
  ...
  it("names the channels when launching immediately", () => {
    const summary = confirmationSummary(
      draftWith({ campaignName: "Q3", channel: "both", selectedLeadIds: ["a"] }),
      { launch: true },
    );
    expect(summary).toMatch(/email and AI calls/i);
