...
describe("wizard shape", () => {
  it("defaults new campaigns to 25 maximum leads", () => {
    ...
    expect(DEFAULT_COPILOT_DRAFT.apolloCreditLimit).toBe(25);
    expect(buildLaunchPayload({ ...DEFAULT_COPILOT_DRAFT, maxLeads: "" }).maxLeads).toBe(25);
    expect(buildLaunchPayload({ ...DEFAULT_COPILOT_DRAFT, channel: "voice", apolloCreditLimit: "" }).apolloCreditLimit).toBe(200);
  });
