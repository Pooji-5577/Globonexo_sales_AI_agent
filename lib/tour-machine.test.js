...
describe("tour content", () => {
  it("covers every area the tour promises to explain", () => {
  ...
  it("sends every step to the page it is describing", () => {
    for (const step of TOUR_STEPS) {
    ...
    expect(routeFor("campaigns")).toBe("/campaigns");
    expect(routeFor("sequences")).toBe("/campaigns/new");
    expect(routeFor("voice")).toBe("/campaigns/new");
    expect(routeFor("inbox")).toBe("/inbox");
    expect(routeFor("meetings")).toBe("/meetings");
    expect(routeFor("analytics")).toBe("/analytics");
    expect(routeFor("billing")).toBe("/billing");
    expect(routeFor("settings")).toBe("/settings");
    expect(routeFor("finish")).toBe("/setup");
  });
  ...
  it("groups consecutive steps on the same route to avoid redundant navigation", () => {
    // Two steps on /campaigns/new should sit next to each other, not bounce
    // the customer away and back again.
    const routes = TOUR_STEPS.map(step => step.route);
    const visits = routes.filter((route, index) => route !== routes[index - 1]);
    expect(visits.length).toBe(new Set(visits).size);
  });
});
