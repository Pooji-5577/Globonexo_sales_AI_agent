...
export const usesEmail = channel => channel === "email" || channel === "both";
export const usesVoice = channel => channel === "voice" || channel === "both";
...
export function blockersForStep(stepId, { draft, integrations, leadCount }) {
  ...
  if (stepId === "connections") {
    if (usesEmail(channel) && !integrations?.gmail?.connected) {
      blockers.push("Connect Gmail before building an email campaign — nothing can send without it.");
    }
...
export function warningsForDraft({ draft, integrations }) {
  ...
  if (!integrations?.calendar?.connected) {
    warnings.push(
      "Google Calendar is not connected. The agent can raise a meeting but cannot book one into your calendar.",
    );
  }
  ...
  if (integrations?.apollo?.status === "not_configured") {
    warnings.push("Apollo enrichment is not enabled, so messages will use only the lead data you already have.");
  }
...
export function buildLaunchPayload(draft, { launch = false } = {}) {
  ...
  return {
    ...
    maxLeads: Number(draft?.maxLeads) || 100,
...
export function hydrateDraft(saved) {
  ...
  if (!["email", "voice", "both"].includes(draft.channel)) draft.channel = "email";
  return draft;
}
...
export function confirmationSummary(draft, { launch }) {
  ...
  const channelLabel = channel === "both" ? "email and AI calls" : channel === "voice" ? "AI calls" : "email";
